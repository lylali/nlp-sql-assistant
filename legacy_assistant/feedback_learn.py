...
SYN_STORE    = "synonyms.json"          # {"token": {"maps_to": ["table.col","col"], "count": N}}
PATTERNS     = "patterns.jsonl"         # JSONL of induced patterns with slots
...
def ingest_feedback_to_corpus() -> Tuple[int, int]:
    """
    Aggregate (q, sql) with counts. Also update synonyms & patterns.
    Returns (changed, total_unique).
    """
    # 1) aggregate corpus
    ...
    for rec in fb_items:
        ...
        index[key] = was + 1
        changed += 1
        # 2) mine synonyms: tokens near explicit column/table mentions
        #    e.g., "unique state in claims" + "SELECT DISTINCT status FROM claims..."
        ...
        # 3) induce pattern: replace literals/values with slots
